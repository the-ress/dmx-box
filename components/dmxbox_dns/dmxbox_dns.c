...
*/
static char *
parse_dns_name(char *raw_name, char *parsed_name, size_t parsed_name_max_len) {
  char *label = raw_name;
...
static int parse_dns_request(
  ...
  for (int i = 0; i < qd_count; i++) {
    ...
    ESP_LOGD(
        TAG,
        "Received type: %d | Class: %d | Question for: %s",
        qd_type,
        qd_class,
        name
    );
    ...
    if (qd_type == QD_TYPE_A) {
      if (strcasecmp(dmxbox_get_default_hostname(), name) == 0 ||
        ...
        ESP_LOGD(
            TAG,
            "Answer with PTR offset: 0x%X and IP 0x%X",
            (unsigned int)ntohs(answer->ptr_offset),
            (unsigned int)ip_info.ip.addr
        );

        answer->addr_len = htons(sizeof(ip_info.ip.addr));
        answer->ip_addr = ip_info.ip.addr;
      } else {
        // ESP_LOGI(TAG, "Received request for %s; ignoring", name);
      }
    }
  }
  return reply_len;
  ...
  while (1) {
    ...
    while (1) {
      ...
      // Data received
        ...
        ESP_LOGD(
            TAG,
            "Received %d bytes from %s | DNS reply with len: %d",
            len,
            addr_str,
            reply_len
        );
