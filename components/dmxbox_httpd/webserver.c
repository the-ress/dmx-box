...
esp_err_t dmxbox_webserver_start(void) {
  ...
  ESP_RETURN_ON_ERROR(httpd_start(&server, &config), __func__,
                      "httpd_start failed");

  ESP_LOGI(TAG, "Registering URI handlers");

  ESP_RETURN_ON_ERROR(dmxbox_api_config_register(server), TAG,
                      "api_config_register failed");

  ESP_RETURN_ON_ERROR(dmxbox_ws_register(server), TAG, "ws_register failed");

  ESP_RETURN_ON_ERROR(dmxbox_api_settings_sta_register(server), TAG,
