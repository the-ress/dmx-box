...
static void send_poll_reply_to_socket(int sock, tcpip_adapter_if_t interface)
{
    ...
    int err = sendto(sock, (uint8_t *)&reply, sizeof(reply), 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
    if (err < 0)
    {
...
static void apply_changes(const uint8_t *last_data, const uint8_t *current_data, uint16_t data_length)
{
...
static void handle_dmx_data(const uint8_t *data, uint16_t data_length, const struct sockaddr_storage *source_addr, const char *addr_str)
{
    uint8_t *last_data;
...
static void handle_op_dmx(int sock, const struct sockaddr_storage *source_addr, const char *addr_str, const uint8_t *packet, int len)
{
    if (LOG_DMX_DATA)
    {
    ...
    {
        ESP_LOGE(TAG, "Data length mismatch: expected %d, got %d", data_length, len - 18);
        return;
    }
    ...
    else
    {
        if (LOG_DMX_DATA)
        {
            ESP_LOGI(TAG, "Received data for unknown universe %d from %s", universe, addr_str);
        }
    }
}
...
{
    button_event_t ev;
    QueueHandle_t button_events = pulled_button_init(PIN_BIT(RESET_BUTTON_GPIO), GPIO_PULLUP_ONLY);
...
void send_poll_reply_on_connect(void *parameter)
{
    while (1)
    {
        EventBits_t bits = xEventGroupWaitBits(wifi_event_group,
                                               WIFI_SHOULD_ADVERTISE_ARTNET_AP | WIFI_SHOULD_ADVERTISE_ARTNET_STA,
                                               pdTRUE,
                                               pdFALSE,
                                               portMAX_DELAY);
...
void artnet_receive_loop(void *parameter)
{
    ...
    while (1)
    {
        ...
        int len = recvfrom(sock, context->packet_buffer, sizeof(context->packet_buffer), 0, (struct sockaddr *)&source_addr, &socklen);
        ...
        handle_packet(sock, context->interface, &source_addr, context->packet_buffer, len);
    }
