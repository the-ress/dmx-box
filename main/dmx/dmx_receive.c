...
static esp_err_t configure_dmx_in(void)
{
    ...
    if (!dmx_set_pin(DMX_IN_NUM, DMX_PIN_NO_CHANGE, DMX_IN_GPIO, DMX_PIN_NO_CHANGE))
    {
        return ESP_FAIL;
    }
...
void dmx_receive_task(void *parameter)
{
    ...
    while (1)
    {
        size_t packet_size = dmx_receive_num(DMX_IN_NUM, &packet_info, DMX_PACKET_SIZE_MAX, DMX_TIMEOUT_TICK);
        ...
        case DMX_ERR_IMPROPER_SLOT:
            ESP_LOGE(TAG, "Received malformed byte at slot %i", (int)packet_info.size);
        ...
        case DMX_ERR_NOT_ENOUGH_SLOTS:
            ESP_LOGE(TAG, "User DMX buffer is too small - received %i slots", (int)packet_info.size);
